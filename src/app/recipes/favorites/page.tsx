...
import { ChevronLeft, Trash2, Utensils, X, AlertTriangle } from "lucide-react";
...
export default function FavoriteRecipesPage() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; item: FavoriteRecipe | null }>({
    isOpen: false,
    item: null,
  });
  ...
  async function removeFavorite(id: string, title?: string) {
    try {
      const response = await fetch(`/api/favorites/recipes?recipeId=${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        setFavorites((prev) => prev.filter((f) => f.recipeId !== id));
        toast.success("Рецепт удален", {
          description: `"${title || "Рецепт"}" удален из избранного`,
        });
      }
    } catch (error) {
      toast.error("Ошибка удаления", {
        description: "Не удалось удалить рецепт из избранного",
      });
    }
  }

  function confirmDelete(item: FavoriteRecipe) {
    setDeleteModal({ isOpen: true, item });
  }

  function handleConfirmDelete() {
    if (deleteModal.item) {
      removeFavorite(deleteModal.item.recipeId, deleteModal.item.recipe?.title);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider">
                {favorites.length}{" "}
                {favorites.length === 1 ? "рецепт" : favorites.length < 5 ? "рецепта" : "рецептов"}
              </p>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {favorites.map((fav) => (
              <div
                key={fav.id}
                className="group bg-[#16181d] border border-white/5 rounded-xl p-4 flex items-center gap-3 hover:border-red-500/30 transition-all"
              >
                <Link href={`/recipes/${fav.recipeId}`} className="flex items-center gap-3 flex-1">
                  <div className="w-12 h-12 rounded-xl bg-red-500/10 flex items-center justify-center shrink-0">
                    <Utensils className="w-6 h-6 text-red-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white truncate">
                      {fav.recipe?.title || `Рецепт ${fav.recipeId}`}
                    </p>
                    <p className="text-xs text-zinc-500">
                      {fav.recipe?.category || "Избранное"} •{" "}
          ...
          <div className="bg-[#16181d] border border-white/5 rounded-2xl p-8 text-center">
            <div className="w-16 h-16 rounded-full bg-[#f95700]/10 flex items-center justify-center mx-auto mb-4">
              <Utensils className="w-8 h-8 text-[#f95700]" />
            </div>
            <p className="text-lg font-bold text-white mb-2">Нет избранных рецептов</p>
            <p className="text-sm text-zinc-500 mb-4">Сохраняйте любимые рецепты в избранное</p>
            <Link
              href="/recipes"
          ...
          <div className="relative bg-[#16181d] border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-zinc-400 mb-6">
                "{deleteModal.item?.recipe?.title || "Рецепт"}" будет удален из вашего списка
                избранного
              </p>
